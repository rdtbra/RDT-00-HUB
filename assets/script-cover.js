 ...
 * ------------------------------------------------------------
 * Arquivo: script-cover.js (CORRIGIDO: Chaves padronizadas)
 * ============================================================
 */
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ✅ CORREÇÃO: Mesmo APP_ID e prefixo do launcher
  const APP_ID = (window.LAUNCHER_APP_ID || "AI-EMT-Equipes").trim();
  const STORAGE_PREFIX = `ia-launcher-config:${APP_ID}`;

  // ✅ CORREÇÃO: Chaves consistentes
  const LS_ITEMS_KEY = `${STORAGE_PREFIX}:items:${groupId}`;
  const LS_DESC_KEY  = `${STORAGE_PREFIX}:desc:${groupId}`;

  // --- 1. Sincronização de Identidade ---
  function loadGroupData() {
    // ✅ CORREÇÃO: Buscar com chave padronizada
    const localHeader = localStorage.getItem(`${STORAGE_PREFIX}:group:${groupId}`);
