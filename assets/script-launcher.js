...
/.test(slug);
    ...
    function openModal(mode, groupData = null) {
        ...
        document.getElementById("mSave").onclick = () => {
            ...
            showFeedback("✅ Grupo salvo!", "success");
            overlay.remove();
            init();
        };
        ...
        activeGroups.forEach(g => {
            ...
            card.innerHTML = `
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    <h2 class="group-name" style="margin:0; font-size:18px; color:${g.color || "#8b86ff"};">${g.name}</h2>
                    <div style="display:flex; gap:10px;">
                        <button class="btn btn-cover" style="background:#444; padding:8px 12px; border-radius:6px;" title="Abrir Capa">📄</button>
                        <button class="btn btn-edit" style="background:#22c55e; padding:8px 12px; border-radius:6px;" title="Editar Grupo">✏️</button>
                        <button class="btn btn-export" style="background:#8b86ff; padding:8px 12px; border-radius:6px;" title="Exportar Grupo">⬇️</button>
                    </div>
                </div>
                <div class="grid" style="display:${g.collapsed ? "none" : "grid"}; grid-template-columns:repeat(auto-fit, minmax(150px, 1fr)); gap:15px; margin-top:20px;">
                    ${g.items.map(item => `
                        <div style="background:#0d0d0d; border:1px solid #333; border-radius:8px; padding:15px; display:flex; align-items:center; justify-content:space-between;">
